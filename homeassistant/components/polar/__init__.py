        ...
        DOMAIN: {
            ...
            CONF_MONITORED_RESOURCES: {
                CONF_DAILY_ACTIVITY: vol.All(
                    cv.ensure_list, [vol.In(RESOURCE_NAMES[CONF_DAILY_ACTIVITY])]
                ),
                CONF_TRAINING_DATA: vol.All(
                    cv.ensure_list, [vol.In(RESOURCE_NAMES[CONF_TRAINING_DATA])]
                ),
                CONF_PHYSICAL_INFO: vol.All(
                    cv.ensure_list, [vol.In(RESOURCE_NAMES[CONF_PHYSICAL_INFO])]
                ),
            },
        }
    },
